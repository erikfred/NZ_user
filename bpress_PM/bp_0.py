...
for sn in sn_list:
    ...
    bv2lp = np.nan * np.ones(sig0.shape)
    bv2lp[:,1:-1] = -(g/rho0) * (sig0[:,2:]-sig0[:,:-2]) / (Z[2:]-Z[:-2]).reshape((1,N-2))
    bv2lp[bv2lp <= 0] = 1e-10
    bvlp = np.sqrt(bv2lp)
    bvlp[:,0] = bvlp[:,1]
    bvlp[:,-1] = bvlp[:,-2]
    ...
    if True:
        ...
        #
        ax = fig.add_subplot(nrow,1,1)
        cs = ax.pcolormesh(tlpf, ZW, ulp_a.T, cmap='RdYlBu_r', vmin=-0.2, vmax=0.2)
        ax.text(.03, .07, 'Low-passed U [m s-1]', transform=ax.transAxes, weight='bold')
        ax.grid(True)
        icb(ax,cs)
        ax.set_xlim(dt[0],dt[-1])
        ax.set_title(sn)
        ...
        ax.set_ylabel('Z [m]')
        #
        bv_lim_dict = {'CE01':1e-2, 'CE02':1e-2, 'CE04':3e-3, 'PN01A':1e-3}
        bv_lim = bv_lim_dict[sn]
        ax = fig.add_subplot(nrow,1,3)
        cs = ax.pcolormesh(tlpf, ZW, bvlp_a.T, cmap='RdYlBu_r', vmin=-bv_lim, vmax=bv_lim)
        ax.text(.03, .07, 'Low-passed Buoyancy Freq. Anomaly [s-1]', transform=ax.transAxes, weight='bold')
        ax.grid(True)
        icb(ax,cs)
        ax.set_xlim(dt[0],dt[-1])
        ax.set_ylabel('Z [m]')
